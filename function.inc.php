...
function get_config_info()
{
    ...
    while ($row = mysql_fetch_assoc($res)) {
        ...
        while ($dev = mysql_fetch_assoc($res1)) {
            ...
            if ($row['序号'] == $dev['栏目号']) {              
				$dev_info[$j]['device_name'] =  $dev['设备名'];
                $dev_info[$j]['icon'] = $dev['图标'];
				$dev_info[$j]['image'] = $dev['监控图片'];
...
function get_allDevice_data()
{
    ...
    $res = mysql_query($sql);
    while ($row = mysql_fetch_assoc($res)) {
		$datas[$j]['device_name'] =$row["设备名"];
		$datas[$j]['image']=$row["监控图片"];
        $delimiters = array(',', '，');
        ...
        foreach ($field_arr as $r) {
			...
			$sql1 = "select  `$r`" . " from " . $row["表名"] . " order by 时间 desc limit ".$row["数据量"];
            $res1 = mysql_query($sql1);
